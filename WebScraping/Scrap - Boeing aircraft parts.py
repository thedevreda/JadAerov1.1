...
def scrape_boeing_parts():
    ...
    for page_num in range(1, 24):
        ...
        page_url = f'https://www.aerospacepurchasing.com/boeing-aircraft-parts-{page_num}/'
        ...
        table_div = soup.find('div', {'class': 'boeing-aircraft-parts-table'})
        ...
        for row in rows:
            ...
            part_number_link = part_number_cell.find('a')
            part_number = part_number_link.find('span').text.strip()
            
            # Get description from second cell
            description_cell = cells[1]
            description_span = description_cell.find('span', {'class': 'table-desc-text'})
            description = description_span.text.strip()
            
            # Get aircraft model from third cell
            aircraft_cell = cells[2]
            aircraft_span = aircraft_cell.find('span', {'class': 'table-desc-text'})
            aircraft_model = aircraft_span.text.strip()
...
pwd = os.getcwd()
file_path = os.path.join(pwd, r'\Data\Data - Boeing_parts.csv')
